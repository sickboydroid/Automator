...
public class Automator extends BaseAutomator {
    ...
    private static final String PACKAGE_NAME = "package_name_of_app";
    ...
    private final Runnable mAutomatorThreadRunnable = new Runnable() {
		...
		while(mContinueThread) {
			// TODO: Enter code that is to be executed
		}
		onStop();
	    }
     ...
     *
     * @see makeOverlayWrapContent()
     * @see makeOverlayMatchParent()
     * @see makeOverlayTouchable()
     * @see makeOverlayNotTouchable()
     * @see createOverlay()
     * @see removeOverlay()
     */
	...
	mHandler.post(new Runnable() {
		@Override
		public void run() {
		    Toast.makeText(getService(), msg, Toast.LENGTH_LONG).show();
		}
	    });
    }
}
